...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const t: any = task;
    if (t?.source_id) {
      const src = await db.get(`SELECT * FROM external_sources WHERE id = ?`, [t.source_id]) as any | undefined;
      if (src?.access_token) {
        const headers: HeadersInit = { Authorization: `Bearer ${src.access_token}` };
        if (src.provider === 'ms_graph_todo') {
          // Microsoft To Do
          const [listId, taskId] = t.external_id && String(t.external_id).includes(':') ? String(t.external_id).split(':',2) : [null, t.external_id];
          ...
          if (useListId && taskId) {
            ...
            await fetch(`https://graph.microsoft.com/v1.0/me/todo/lists/${useListId}/tasks/${taskId}`, { method: 'PATCH', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).catch(()=>{});
          }
          ...
          if (t.external_id) await fetch(`https://graph.microsoft.com/v1.0/me/events/${t.external_id}`, { method: 'PATCH', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).catch(()=>{});
          ...
          if (useListId && taskId) {
            ...
            await fetch(`https://www.googleapis.com/tasks/v1/lists/${useListId}/tasks/${taskId}`, { method: 'PATCH', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).catch(()=>{});
          }
          ...
          if (t.external_id) await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events/${t.external_id}`, { method: 'PATCH', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).catch(()=>{});
        }
      }
    }
  } catch {}
    ...
    if (t?.source_id && t?.external_id) {
      ...
      if (src?.access_token) {
        ...
        if (src.provider === 'ms_graph_todo') {
          ...
          if (listId && taskId) await fetch(`https://graph.microsoft.com/v1.0/me/todo/lists/${listId}/tasks/${taskId}`, { method: 'DELETE', headers }).catch(()=>{});
        } else if (src.provider === 'ms_graph_calendar') {
          await fetch(`https://graph.microsoft.com/v1.0/me/events/${t.external_id}`, { method: 'DELETE', headers }).catch(()=>{});
        } else if (src.provider === 'google_tasks') {
          const [listId, taskId] = String(t.external_id).includes(':') ? String(t.external_id).split(':',2) : [null, t.external_id];
          if (listId && taskId) await fetch(`https://www.googleapis.com/tasks/v1/lists/${listId}/tasks/${taskId}`, { method: 'DELETE', headers }).catch(()=>{});
        } else if (src.provider === 'google_calendar') {
          await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events/${t.external_id}`, { method: 'DELETE', headers }).catch(()=>{});
        }
      }
    }
  } catch {}

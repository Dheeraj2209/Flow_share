...
async function importICS(url: string, person_id: number, source_id: number) {
  ...
  for (const t of tasks) {
    const existing = (await db.get(
      `SELECT id FROM tasks WHERE source_id = ? AND external_id = ?`,
      [source_id, t.external_id],
    )) as RowId | undefined;
    if (existing?.id) {
      await db.run(
        `UPDATE tasks SET title=?, due_date=?, due_time=?, bucket_type='day', bucket_date=? WHERE id = ?`,
        [t.title, t.due_date, t.due_time, t.due_date, existing.id],
      );
...
export async function POST(req: NextRequest) {
  ...
  if (
    src.provider === "ms_graph_todo" ||
    src.provider === "ms_graph_calendar"
  ) {
    ...
    if (src.provider === "ms_graph_todo") {
      const lists: MsTodoLists = await fetch(
      ...
      if (lists?.value?.length) {
        ...
        for (const list of lists.value) {
          const items: MsTodoItems = await fetch(
          ...
          if (items?.value?.length) {
            for (const it of items.value) {
              ...
              const timePart =
                dtStr && dtStr.length >= 16 ? dtStr.slice(11, 16) : null; // HH:mm
              ...
              if (selectRow?.id) {
                await db.run(
                  `UPDATE tasks SET title=?, due_date=?, due_time=?, bucket_type='day', bucket_date=? WHERE id=?`,
                  [title, due_date, due_time, due_date, selectRow.id],
                );
                const task = await db.get(`SELECT * FROM tasks WHERE id = ?`, [
                  selectRow.id,
                ]);
                ...
                const info = await db.run(
                  `INSERT INTO tasks (title, person_id, status, due_date, due_time, bucket_type, bucket_date, recurrence, interval, sort, source_id, external_id)
                  VALUES (?, ?, 'todo', ?, ?, 'day', ?, 'none', 1, 0, ?, ?)`,
                  [
                    title,
                    src.person_id,
        ...
        const localTasks = (await db.query(
          `SELECT * FROM tasks WHERE source_id = ?`,
          [src.id],
        )) as any[];
        for (const t of localTasks) {
          ...
          if (!t.external_id) {
            ...
            if (created?.id) {
              await db.run(`UPDATE tasks SET external_id = ? WHERE id = ?`, [
                `${firstListId}:${created.id}`,
                t.id,
              ]);
            ...
            await fetch(
              `https://graph.microsoft.com/v1.0/me/todo/lists/${listToUse}/tasks/${taskId}`,
              {
                method: "PATCH",
                headers: { ...headers, "Content-Type": "application/json" },
                body: JSON.stringify(body),
      ...
      const events: MsEvents = await fetch(
    ...
    src.provider === "google_tasks" ||
    src.provider === "google_calendar"
  ) {
    if (!src.access_token)
      return new Response("Not authorized", { status: 401 });
    const headers: HeadersInit = {
    ...
    if (src.provider === "google_tasks") {
      const lists: GTaskLists = await fetch(
      ...
      if (lists?.items?.length) {
        ...
        for (const list of lists.items) {
          const tasks: GTasks = await fetch(
          ...
          if (tasks?.items?.length) {
            for (const it of tasks.items) {
              ...
              const due_time =
                due && due.length >= 16 ? due.slice(11, 16) : null;
              const external_id = `${list.id}:${it.id}`;
              if (!due_date) continue;
              const selectRow = (await db.get(
                `SELECT id FROM tasks WHERE source_id=? AND external_id=?`,
        ...
        const localTasks = (await db.query(
          `SELECT * FROM tasks WHERE (source_id = ? OR (source_id IS NULL AND person_id = ?))`,
          [src.id, src.person_id],
        )) as any[];
        for (const t of localTasks) {
          if (!t.external_id) {
            const body: any = { title: t.title };
            ...
            if (created?.id) {
              await db.run(
                `UPDATE tasks SET source_id = COALESCE(source_id, ?), external_id = ? WHERE id = ?`,
                [src.id, `${firstListId}:${created.id}`, t.id],
              );
              const task = await db.get(`SELECT * FROM tasks WHERE id = ?`, [
                t.id,
              ]);
            ...
            const [listIdRaw, taskId] = String(t.external_id).includes(":")
              ? String(t.external_id).split(":", 2)
            ...
            if (t.source_id == null)
              await db.run(`UPDATE tasks SET source_id = ? WHERE id = ?`, [
                src.id,
                t.id,
              ]);
      ...
      if (events?.items?.length) {
        for (const ev of events.items) {
          ...
          const due_time =
            dt.length >= 16 && dt.includes("T") ? dt.slice(11, 16) : null;
  ...
  return Response.json(
    { ok: true, result: { imported: result.imported, exported } },
    { headers: corsHeaders() },
  );
}
